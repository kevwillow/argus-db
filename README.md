...
| `exports/argus_export.json` | 1,017 | Broader scanner watchlists. Looser confidence floor (≥30); US scope filter. |
...
The part that matters if you run a scanner: **the standard feed grows by 34 entries and one pre-existing entry's stem is re-rendered (autel → autelevo + autelrobotics, +1 net).** The active count moves 43,088 → **43,126**, a clean +38 with no withdrawals, so every v1.7.0 feed entry is still present. Migration `0059` admits 38 rows and only 15 of them reach the feed: 18 `ble_local_name` rows drop at the CP50 template gate and 5 `ssid_pattern` rows route to `ssid_pattern_fp_hold` (1 was already FP-held by `xg`/`harris` shape from the harvest; the 4 added under MAC-752 are `flock`, `digital[_-]?ally`, `msab|xry`, and `stingray`, each of which was emitting a bare word or a 3-4 char token that matched ordinary network names; the `wifi[_-]?pineapple` branch of the Hak5 pattern is rendered fully as `wifipineapple` rather than the bare `wifi`). That is the false-positive discipline from v1.7.0 doing its job on a fresh harvest, not a shortfall.

Feed movement: the standard Lynceus feed **983 → 1,017**; high-confidence **501 → 504**; the behavioral feed holds at **132**. `schema_version` is unchanged at **35**, because neither migration carries DDL.
