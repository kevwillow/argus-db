...
Argus **v1.4.0** ships at schema_version=24 with:

- **34,792 active canonical identifiers** + 80 superseded (kept for audit-trail) across 15 user tables (net +12,239 in v1.4.0 from the Wave I/I.5/I.6/I.7 vendor cloud-infrastructure hostname corpus integration)
...
- **146,188 raw observations** with per-row source provenance (every active identifier traceable to at least one source citation; +12,358 in v1.4.0 from Wave I cumulative attestation rows)
- **116,774 deployment_observations** from EFF Atlas of Surveillance + DeFlock with per-row LICENSE column for downstream license-aware filtering (+106 multi-tenant subdomain pattern rows in v1.4.0)
- **66 upstream sources** across canonical registries, procurement data, academic research, manufacturer documentation, community-OSINT GitHub repositories, corporate registries, judicial filings, federal disclosure / entity-registration sources, and (new in v1.4.0) Certificate Transparency Logs (crt.sh aggregator), Internet Archive Wayback Machine, GitHub vendor first-party content, 5 Regional Internet Registry RDAP endpoints (ARIN/RIPE/APNIC/LACNIC/AFRINIC), NPM/PyPI/RubyGems public package registries, vendor public cloud-storage bucket payloads, and the Wave I extraction methodology umbrella (13 net-new sources)
- **51 surveillance-tech vendors** in the canonical lexicon (unchanged in v1.4.0; +6 novel manufacturer aliases appended from live cert Subject DN O + firmware cert observations — Autel Intelligent Technology Corp., Axis Communications AB, Meraki LLC, Getac Technology Corporation, Jacobs Solutions Inc., Honeywell International Inc.)

**Vendor cloud-infrastructure hostname corpus landed in v1.4.0** — CP29 codifies three new identifier_types (`vendor_controlled_hostname`, `vendor_cloud_endpoint_url`, `vendor_controlled_hostname_deprecated`) surfaced by Wave I/I.5/I.6/I.7 across 8 extraction source-classes (crt.sh CT logs at 11,551 attestations dominant + binary static analysis + GitHub source/README + S3 misconfig + subdomain enumeration + cloud doc URL space + bucket payload + package registry). Marquee finding: **`hppki.honeywell.com`** promoted at confidence=99 (firmware-cert ceiling) via 4-source independent corroboration including 2 Honeywell OTA signing certs recovered from CT40 Android firmware (Honeywell CodeSign RSA CA / OU=ACS / O=Honeywell International Inc.). 565 NXDOMAIN-verified deprecated hostnames promoted as historical-attribution anchors at confidence=87. 108 cross-source §8.3 lifts applied per CP24 independence verification. SAR-13 (runguide-schema-fabrication discipline) and SAR-13.5 (bucket-attribution-FP discipline) codified as sibling bible amendments. See CHANGELOG v1.4.0 + BIBLE_AMENDMENTS.md CP29 / SAR-13 / SAR-13.5 for full detail.
