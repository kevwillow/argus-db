...
- **Single-product §2.1 vendor OUI categorization at export time — narrow §11 #10 / §5 Tier 4 reading vs strict §8.4.** Decide at CP5 with empirical row-count visibility per disposition. §8.4 broad rule was added specifically to prevent FP cascades from OUI-level categorization (e.g., flagging every Flock Audio device as a Flock Safety camera). The Tier 4 example #1 (single-product manufacturer body_cam inference) and §11 #10 narrow read ("multi-purpose vendors only") are real but the strict §8.4 conservative ratchet was approved for v1 Phase-5 Step-4 (MAC-39, MAC-1 [`613ec532`](/MAC/issues/MAC-1#comment-613ec532-d8cb-4f0f-a35b-c811e2864d7d) 2026-05-06): Talos false positives on ambiguous OUI categorization are much worse than missing some single-product vendor categorizations. **CP5 must surface row counts per disposition** — how many records flip categorization under narrow read vs strict §8.4 — as the load-bearing input for the eventual decision. Single-product §2.1 vendors include: Flock=alpr / DJI=drone / Skydio=drone / Hak5=hacking_tool / Cellebrite=hacking_tool / ShotSpotter=gunshot_detect. (Added by SAR-8 board ratification 2026-05-06.)
