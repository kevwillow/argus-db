-- ============================================================================
-- Migration: 0033_cp46_device_category_smart_lock_smart_home_hub
-- Issue:     MAC-419 (child of MAC-392) · Author: CTO (0715773f)
-- Applied:   2026-06-15 (board-AUTHORIZED ingestion; gate #3 push CEO-owned — NO push)
-- CP:        CP46. Next-free CP above landed maximum CP45 (migration 0032,
--            bluetooth_tracker, MAC-388). The parked MAC-359 "CP46" draft was
--            SUPERSEDED into the existing CP21 export MAP ("no separate CP46
--            lands" — BIBLE_AMENDMENTS CP45 Part-C reconciliation), FREEING the
--            CP46 slot. MAC-360's "CP47" draft (ble_company_id normalize, STAGED,
--            not landed) and MAC-416's anticipated >=CP48 (§11 #22) are untouched.
--            Verified next-free by direct working-tree read (CP-slot-reservation).
--            schema_version row = 33 (file 0033).
-- Purpose:   Extend `device_category` CHECK enum +2 net-new values on BOTH host
--            tables (identifiers + behavioral_signatures), maintaining the
--            CP32/CP33/CP37/CP45 dual-table parity invariant:
--              - `smart_lock`     (consumer/commercial BLE smart locks: Kwikset /
--                 Schlage / August-Yale / Ultraloq — MAC-393 cohort-4).
--              - `smart_home_hub` (coarse durable home for smart-home hubs;
--                 carries SmartThings + future hub identifiers — cohort-5).
-- Rationale: Board pre-ingestion ratification (MAC-392, accepted 2026-06-15 00:10)
--            of the wave-2 disposition slate. cohort-4 = 56 corroborated byte-
--            faithful net-new identifiers across 4 pure-play lock vendors; cohort-5
--            = SmartThings hub OUI. `voice_assistant` HELD (empty) and `wearable`
--            DEFERRED (privacy one-way-door, zero net-new) — neither minted here.
-- Bible:     §11 #1  no fabrication — concrete-vendor cites; only board-approved
--                     cite-verified rows promoted.
--            §11 #11 amendment-log discipline — paired CP46 entry in
--                     docs/engineering/BIBLE_AMENDMENTS.md (no silent CP).
--            §11 #13 PROMOTING-category carve-out (parity CP37/CP45): rows in
--                     smart_lock / smart_home_hub are not §11 #13 export-banned;
--                     §4.4 type-map + conf/source/CP7 gates still apply.
--            SAR-13 sqlite_master-first — _new DDL GENERATED column-for-column
--                     from LIVE post-0032 sqlite_master (single-token enum
--                     injection, occurrence-count asserted == 1), NOT transcribed.
-- Pattern:   SQLite table-rebuild (CP21 cumulative-full-enum), mirrors 0032.
-- Idempotency: apply script skips the rebuild when the enum already contains
--            'smart_lock'. schema_version insert is OR IGNORE.
-- ============================================================================
...
-- ─── identifiers table rebuild (device_category +2: smart_lock, smart_home_hub) ─
...
-- ─── behavioral_signatures table rebuild (device_category +2, parity) ────────
...
-- Version bump — CP46 / file 0033.
INSERT OR IGNORE INTO schema_version (version, name) VALUES
    (33, '0033_cp46_device_category_smart_lock_smart_home_hub');
