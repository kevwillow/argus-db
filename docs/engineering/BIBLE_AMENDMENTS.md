...
This SAR-15 entry is the bible-amendment sibling of the MAC-183 post-ship audit (board comment `ddfb43d2-d87a-4fba-a12d-352b539f79fb`, 2026-05-20T00:54:23Z) + post-ship corrective commit `64efd6f` + the 252-row JM scope-review-flag UPDATE pass.
...
**Disposition (this cycle):** Honeywell alias appended post-ship via corrective commit `64efd6f`. SAR-15.5 codifies the methodology so the next big-ship cycle (e.g., Wave I-prime / v1.5.0) routes through a Validator close-out audit by default.
...
**Forward-looking architectural binding (CP31 surface).** When a future migration adds `identifiers.manufacturer_id` as an FK (pre-staged by CP31's hub-and-spoke columns), the export-path JOIN MUST re-establish the visible-filter as `WHERE m.query_default = 'visible' OR id.manufacturer_id = m.id`. This binding is the canonical architectural commitment of CP31; future migration-design proposals consuming `identifiers.manufacturer_id` must paste-not-cite this requirement in their dispatch §1. Documented at `_phase_cp31_implementation/manufacturers_query_audit.md` §E in MAC-199 commit `9a4eee0`.
...
4. **`project_knowledge_search` / live-query path audit** — 28 occurrences of `FROM manufacturers` / `JOIN manufacturers` classified at MAC-199. 4 hub-only live-query lexicon enumeration sites received the `WHERE query_default = 'visible'` filter in commit `9a4eee0`:
...
- **Migration 0025 + Parrot conversion** (MAC-198 DBArchitect): commit `29abb27`
- **4-path consumer audit + 8 new tests** (MAC-199 Validator): commit `9a4eee0`
...
- **Commit 1 — `git mv 0026_phase10_*.sql 0026a_phase10_*.sql`** (Na_ sub-slot rename): `c1ec6a5`
- **Commit 2 — Migration 0026 CP32 §1 schema landing**: `9666a78`
- **Commit 3 — CP32 §3 test refactor + DROPPED_REASONS stubs**: `6874f84`
...
- MAC-200 Phase 9 precedent: `_phase_9_wave_i_13_carry_forward/heartbeat.md` §9.2.c (commit `2a49bab`) — same sandbox surfaced as cleaned/unavailable.
...
This entry stages §11 #18 as draft. §11 #11 self-binding activates at CP34 ratification: the consolidated commit hash applying the §11 #18 amendment to `PROJECT_BIBLE.md` §11 must be enumerated here at ratification time. Ratified at commit `fc50a341bb1b4f2b0fadfa551b6ecdeb73a9a702` (MAC-301, CP41 ratification commit, 2026-06-03); this §11 #18 amendment was promoted to canonical §11 #20 at apply-time per CP41 Apply-time correction; §11 #11 self-binding active.
...
### MAC-535 — §11 #21 reservation: idempotency-by-default for un-backstopped UPDATEs in migrations; §6.2 alias-tokenization defense (Finding 1 of MAC-533 §cto_ratification.md)
...
**Rule (new §11 hard rule — `§11 #21` — migration UPDATE idempotency-by-default).** Any `UPDATE` statement in a `db/migrations/*.sql` file that lacks a UNIQUE / NOT-NULL / CHECK backstop on the mutated column(s) MUST be written idempotently. The canonical idempotency shape is one of:
  ...
  (b) **Full-value overwrite.** `UPDATE ... SET col = <full computed expression>` where the expression is deterministically derived from a snapshot of the row's pre-migration state (e.g., read the canonical alias list from a `migrations/_aliases_snapshot` reference table or the migration file's own payload). Catches re-apply because the second write produces the same final value.
  (c) **Re-apply abort with transactional guard.** `BEGIN TRANSACTION; ... pre-condition CHECK(ok=1) on a TEMP table ...; UPDATE ...; COMMIT;` where the pre-condition fails (second apply) and aborts the WHOLE transaction (requires the `sqlite3 -bail` CLI flag at apply time, OR explicit `PRAGMA recursive_triggers=ON` / explicit `RAISE(ABORT, ...)` traps on the failing condition). Note: this shape is fragile in `sqlite3` CLI without `-bail` because the CLI does not auto-rollback on CHECK violation by default — see Finding 2 lesson.

The discipline is binding on every future migration authored. Canonical rejection pattern: an `UPDATE` that mutates a column without a UNIQUE/CHECK backstop AND without one of the three idempotency shapes above is a CTO-ratification halt at code-review time. Rationale: re-apply safety was historically proven "by mental simulation"; mig-0041 proves simulation misses silent double-writes when (a) UNIQUE constraints backstop most of the script and (b) one statement slips through without one. Re-apply safety MUST be proven by executing a second apply on a throwaway copy and diffing non-count fields (counts alone would have missed this — row counts were correct on the doubled-apply).

**Sibling rule (proposed §11 #22 if a future dispatch codifies it):** Re-apply safety discipline — every migration's README / header note MUST cite the proof shape (throwaway-copy double-apply diff) used at authoring time. mig-0041's amended header now carries this discipline inline.

**Finding 1 fix shape (deployed at MAC-535).** `db/validation/coverage_matrix.py::_alias_tokens_for_vendor` rewritten with a 3-layer defense:
  1. **Quote-aware split** (RFC-4180-lite) — current data is unquoted so this is a no-op today, but the parser handles future quoted values correctly without code change.
  2. **Corporate-suffix stop-list** (case-insensitive) — drops `ltd`, `ltd.`, `inc`, `inc.`, `llc`, `co.`, `co`, `the`. Catalogue copied verbatim from `operator_review/MAC-533/cto_ratification.md §Finding 2`.
  3. **Min-length floor** — drops tokens of length ≤3 (catches future short-token false positives like "L3").
Before/after corroboration table for all 17 affected vendors is in `operator_review/MAC-535/tokenization_analysis.md` (also ratified in `fix_proof.md`). 13 of 17 vendors flip from `high_corroboration=True` → `False`; the 4 that stay True (DJI 50, Motorola Solutions 32, Axon 23, Autel Robotics 10) are genuinely above the HIGH_CORROBORATION_GRANTEES_FLOOR=10 threshold. **No `identifiers` row or confidence value is affected** (§6.2 corroboration is explicitly NOT a §8.3 uplift channel per `coverage_matrix.py`:1205). Tier flips are the *point* of the fix, not a regression.

**Finding 2 fix shape (deployed at MAC-535).** `db/migrations/0041_*.sql` body preserved verbatim (amending an applied migration breaks the provenance link between the file and what actually ran per `cto_ratification.md §Finding 1` discipline); only a `-- WARNING: NOT RE-APPLY-SAFE` header note added naming (a) the specific failure mode (the 6 `UPDATE manufacturers SET aliases = ...` on id=209), (b) the root cause (sqlite3 CLI does not `-bail` by default; CHECK violation aborts the statement not the script), and (c) the two mitigation shapes if re-apply ever becomes operationally necessary (token-guard append OR full-value overwrite per §11 #21 sub-rules (a)/(b)). Canonical `db/argus.db` is clean (re-queried 2026-07-28; 10 tokens on Hikvision 209, no duplicates) — this is a latent hazard only, not a live defect.

**Authority.** DBArchitect (this commit author, agent `6c93a466-d498-49e0-b7af-3fc0d08eb2b0`) under CTO dispatch MAC-535. §11 #21 text drafted in this entry; PROJECT_BIBLE.md §11 §-text insertion deferred to the next coordinated CP cycle per §11 #11 amendment-log discipline (the amendment-log entry IS the closure; the §-text insert is a downstream apply-time concern). Re-apply-safety discipline is the binding output for future migrations; the §-text insertion is mechanical.

**Companion artifacts.**
- `operator_review/MAC-535/tokenization_analysis.md` — Finding 1 design justification + 17-vendor before/after corroboration table + the case against the alternatives the issue cited (JSON-array storage, stop-list-only, min-length-only).
- `operator_review/MAC-535/fix_proof.md` — coverage_matrix test results (43/43 pass; 11 new + 32 pre-existing), mig-0041 WARNING header note text, §11 #21 slot reservation proof (working-tree read of Bible HEAD confirms max=§11 #20, #21 next-free).
- `tests/test_coverage_matrix.py` — 11 new tests under the `MAC-535 §6.2 alias-tokenization defense` section header, covering each layer of the 3-layer defense plus end-to-end on Hikvision/Motorola/WatchGuard.
- `db/validation/coverage_matrix.py` — 3-layer tokenization fix; existing 32 tests unchanged.
- `db/migrations/0041_mac533_ipvm_public_directory_attribution_ingest.sql` — WARNING header note added; body UNCHANGED.
