...
Argus organizes sources by **`source_type`**, a schema-level enum that determines the confidence ceiling each source can contribute (see §5 for confidence bands). The v1.0.0 schema enum (post-migration 0015) carried 10 values; CP23 (mig-0020) added three judicial / corporate-disclosure / vendor-procurement-disclosure source-types, and CP36 (mig-0029) closed the `identifiers.source_type` ↔ `sources.source_type` enum-parity gap so both tables carry the same 13-value CHECK at v1.6.2 ship (verified at `db/argus.db` HEAD: 13/13 on both tables; 9 populated on `identifiers`, 10 populated on `sources`). Listed in descending order of active-identifier representation at v1.0.0:
