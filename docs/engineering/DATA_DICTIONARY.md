...
Current schema_version: **27** (post-mig-0027 CP33, v1.5.0 ship state). `identifier_type` enum: 57 values. `device_category` enum: 16 values (dual-table parity on `identifiers` + `behavioral_signatures` per the CP32 §1 sweep precedent). `pair_kind` enum: 5 values. `source_type` enum: 13 values. Manufacturer arm rows: 2 (Parrot Automotive id=222 under Parrot id=25; Pelco id=254 under Motorola Solutions id=3). Migration ledger: 0001 through 0027 (plus the `Na_` sub-slot addendum 0026a from CP32).
...
The v1.0.0 schema carries **15 tables**; cumulative through `schema_version=27` (post-CP33; the v1.5.0 ship-state). They group into four functional categories:
...
Verified against `db/argus.db` 2026-05-22 (post-CP33 ratification on branch `v1.5.0-integration-stage-1`; HEAD `ff066f1`; `SELECT COUNT(*)` over each table; values probe-verified at fact-lock time per `_stage2_facts.json`):

| Table | Row count | Notes |
|---|---:|---|
| `identifiers` | **35,812 active** + chained-superseded + self-loop withdrawn-no-successor | active = `superseded_by IS NULL` = **35,812**; CP32 §9 tri-state semantic preserved (`NULL` = active / `<other_id>` = superseded by successor / `<self_id>` = withdrawn-no-successor PII-demote). Cumulative active deltas: +1 in v1.1.0 (Johnson Matthey PLC); +16 in v1.2.0 (MAC-178 P3); +12,239 in v1.4.0 (Wave I/I.5/I.6/I.7 hostname corpus); +172 in v1.4.1 (Stage 1 SAR-15.5 PASS); **+848 in v1.5.0** (Stage 1 Step 5 identifier promotions via [MAC-232](/MAC/issues/MAC-232) lexicon-expansion wave) |
| `raw_observations` | (append-only) | provenance source-of-truth |
| `sources` | **73** | source registry (+2 in v1.5.0: sid=72 GitHub Code Search REST API + sid=73 adsb.lol v2 FAA-registry-derived aircraft tracking; admitted under the v1.5.0 lexicon-expansion wave per [MAC-232](/MAC/issues/MAC-232) and CP33 §1. Cumulative: +5 in v1.4.1 vendor APK sources via mig-0026a — sids 67-71) |
| `manufacturers` | **92** | vendor metadata lookup (+40 net in v1.5.0: 21 Session 1 military/federal + 19 Session 2 commercial/consumer — Pelco arm counted within Session 2 camera_vms cohort. **2 arms total**: Parrot Automotive id=222 under Parrot id=25 (v1.4.1; first arm) + Pelco id=254 under Motorola Solutions id=3 (v1.5.0; second arm per gate G-A). Hub-visible split: 90 `query_default='visible'` + 2 `query_default='hidden_arm'`). Retroactive cctv_camera recategorization per gate G-B Step 6 flipped 7 existing vendors (Hikvision/Dahua/Axis/Avigilon/Verkada/Eagle Eye Networks/Rhombus Systems) to `primary_category='cctv_camera'`; 31 identifier rows recategorized |
...
| `behavioral_signatures` | **201** | parametric metadata (added migration 0010); CP33 §2 mig-0027 extended `device_category` CHECK enum 13 → 16 (`+cctv_camera`, `+persistent_surveillance`, `+through_wall_radar`) — second dual-table CHECK literal sweep continuing the CP32 §1 precedent; 0 row promotions land at v1.5.0 (schema slots open for future evidence-arrival) |
| `conflicts` | (validator-side disputed rows) | |
| `extraction_runs` | (per-run telemetry) | |
| `source_reclassifications` | (per-row band-correction audit; added migration 0017) | |
...
| `schema_version` | **27** | migration ledger; one row per applied schema-mutating migration. mig-0026a (v1.4.1 `Na_` sub-slot data-only addendum) does NOT bump `schema_version` (filename↔schema_version 1:1 holds for schema-mutating migrations only); mig-0027 is the v1.5.0 schema-mutating migration that bumped the ledger |
...
The canonical Argus identifier table. Every row represents one identifier-to-attribution binding. Row count verified live 2026-05-22 (post-CP33, branch `v1.5.0-integration-stage-1`, HEAD `ff066f1`): **35,812 active** (`superseded_by IS NULL`); chained-superseded (`superseded_by` points at a successor row) + self-loop withdrawn-no-successor (`superseded_by = id`; the §11 #3 PII-demote semantic codified at CP32 §9) rows preserved per the CP32 §9 tri-state semantic. v1.5.0 net delta: +848 active from Stage 1 Step 5 identifier promotions via [MAC-232](/MAC/issues/MAC-232) lexicon-expansion wave.
...
Source registry. FK target for `raw_observations.source_id`. Row count verified live 2026-05-22 (post-CP33 v1.5.0 ship-state on branch `v1.5.0-integration-stage-1`): **73** (+7 in v1.1.0; +2 in v1.2.0: fccid.io sid=51 + FCC Equipment Authorization System — Filings sid=52; +1 in v1.3.0: Wave H sid=53; +13 in v1.4.0: Wave I cumulative source admissions sids 54-66; +5 in v1.4.1: vendor companion APK admissions sids 67-71 via mig-0026a `Na_` sub-slot; **+2 in v1.5.0**: sid=72 GitHub Code Search REST API (`crowdsourced`, tier 3) + sid=73 adsb.lol v2 FAA-registry-derived aircraft tracking (`regulatory`, tier 3), both admitted via the v1.5.0 lexicon-expansion wave per [MAC-232](/MAC/issues/MAC-232) and CP33 §1. Two existing sources cited heavily but NOT re-admitted per §11 #11 dedup-merge discipline: sid=51 fccid.io (60 v1.5.0 citations) + sid=54 crt.sh aggregator (747 v1.5.0 citations).
...
Vendor metadata + alias canonicalization. Row count verified live 2026-05-22 (post-CP33, branch `v1.5.0-integration-stage-1`, HEAD `ff066f1`): **92** total (90 `query_default='visible'` + 2 `query_default='hidden_arm'`). Cumulative: +1 in v1.1.0 (Johnson Matthey PLC); +14 in v1.2.0 (MAC-178 wave); +2 in v1.3.0 (Wave H Cohort A stubs — Eagle Eye Networks, Rhombus Systems); +1 in v1.4.1 (Parrot Automotive id=222 — first multi-arm `hidden_arm` row under CP31 hub-and-spoke schema); **+40 net in v1.5.0** (21 Session 1 military/federal + 19 Session 2 commercial/consumer; Pelco id=254 admitted as second multi-arm `hidden_arm` row under Motorola Solutions id=3 per gate G-A, `is_arm=1, parent_manufacturer_id=3, query_default='hidden_arm', primary_category='cctv_camera'`; SEC Ex21 FY2025 evidence chain). Retroactive cctv_camera recategorization per gate G-B Step 6 flipped 7 existing manufacturers (Hikvision/Dahua/Axis/Avigilon/Verkada/Eagle Eye Networks/Rhombus Systems) to `primary_category='cctv_camera'`; 31 identifier rows recategorized. BriefCam (id=31) DEFERRED per board (analytics-layer ambiguity); `primary_category='face_recog'` unchanged.
...
Parametric metadata for behavioral-class detection signatures. Introduced in migration 0010. Row count verified live 2026-05-22 (post-CP33): **201** (unchanged from v1.4.1; v1.0.0 baseline 131 from the Marlin NDSS 2025 corpus + subsequent Wave I.x backfills carried into v1.4.x; CP33 §2 mig-0027 extended `device_category` CHECK enum 13 → 16 for `+cctv_camera`, `+persistent_surveillance`, `+through_wall_radar` enum parity with `identifiers.device_category` (second dual-table CHECK literal sweep continuing the CP32 §1 precedent), but 0 row promotions land at v1.5.0 — the schema slots open for future evidence-arrival).
...
| `device_category` | TEXT | yes | — | **CHECK constraint** 16 values mirroring `identifiers.device_category` post-mig-0027 / CP33 §2: `alpr`, `imsi_catcher`, `body_cam`, `police_radio`, `drone`, `gunshot_detect`, `hacking_tool`, `covert_cam`, `gps_tracker`, `face_recog`, `drone_detect`, `unknown`, `automotive_telematics`, `cctv_camera`, `persistent_surveillance`, `through_wall_radar`. The CP33 §2 dual-table CHECK literal sweep (continuing the CP32 §1 precedent) maintains enum parity with `identifiers.device_category`; downstream consumers (Lynceus, exports, coverage matrix) treat `device_category` as a single conceptual vocabulary regardless of host table. |
...
| Column | Type | NOT NULL | Default | Description |
|---|---|---|---|---|
| `version` | INTEGER | yes (PK) | — | Migration version number (sequential; current `MAX(version)=27` at the v1.5.0 ship state, verified live 2026-05-22). |
| `name` | TEXT | yes | — | Human-readable migration name (e.g., `'0027_cp33_cctv_camera_persistent_surveillance_through_wall_radar_imei_tac'`, `'0026_cp32_device_category_automotive_telematics'`, `'0025_cp31_fcc_eas_identifier_type_cluster_plus_hub_and_spoke'`, `'0024_cp29_vendor_hostname_corpus_value_classes'`, `'0023_identifier_type_check_extension_cp28'`, …). Full ledger 0001–0027 enumerated below. Data-only addendum migrations using the `Na_` sub-slot convention (e.g., `0026a_phase10_vendor_apk_sources_admission`) do NOT register a `schema_version` row — they apply alongside the schema-mutating `N_` migration sharing the same numeric slot and modify data only. |
| `applied_at` | DATETIME | yes | `CURRENT_TIMESTAMP` | UTC timestamp of migration application. |

Live migration ledger at `schema_version=27` (verified live 2026-05-22):
