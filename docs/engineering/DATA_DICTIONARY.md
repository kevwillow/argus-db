...
Verified against `db/argus.db` 2026-05-25 (post-CP38; v1.6.0 ship state):

| Table | Row count | Notes |
|---|---:|---|
| `identifiers` | **41,428 active** + chained-superseded + self-loop withdrawn-no-successor | active = `superseded_by IS NULL` = **41,428**; CP32 §9 tri-state semantic preserved (`NULL` = active / `<other_id>` = superseded by successor / `<self_id>` = withdrawn-no-successor PII-demote). Cumulative active deltas: +1 in v1.1.0 (Johnson Matthey PLC); +16 in v1.2.0 (MAC-178 P3); +12,239 in v1.4.0 (Wave I/I.5/I.6/I.7 hostname corpus); +172 in v1.4.1 (Stage 1 SAR-15.5 PASS); +848 in v1.5.0 (Stage 1 Step 5 promotions via [MAC-232](/MAC/issues/MAC-232) lexicon-expansion wave); +146 in v1.5.2 (CCTV cohort); +1,843 in v1.5.3 (Phase H, [MAC-250](/MAC/issues/MAC-250)); **+3,627 in v1.6.0** (Wave K + Wave L combined ship, [MAC-275](/MAC/issues/MAC-275)) |
...
| `manufacturers` | **126** | vendor metadata lookup (+34 net in v1.6.0 across the face_recog / spyware / lawful-intercept / forensic / counter-UAS cohorts, incl. 5 CP31 arm-splits — 7 arm rows total. +40 net in v1.5.0: 21 Session 1 military/federal + 19 Session 2 commercial/consumer — Pelco arm counted within Session 2 camera_vms cohort. **2 arms total**: Parrot Automotive id=222 under Parrot id=25 (v1.4.1; first arm) + Pelco id=254 under Motorola Solutions id=3 (v1.5.0; second arm per gate G-A). Hub-visible split: 90 `query_default='visible'` + 2 `query_default='hidden_arm'`). Retroactive cctv_camera recategorization per gate G-B Step 6 flipped 7 existing vendors (Hikvision/Dahua/Axis/Avigilon/Verkada/Eagle Eye Networks/Rhombus Systems) to `primary_category='cctv_camera'`; 31 identifier rows recategorized |
...
| `behavioral_signatures` | **201** | parametric metadata (added migration 0010); CP33 §2 mig-0027 extended `device_category` CHECK enum 13 → 16 (`+cctv_camera`, `+persistent_surveillance`, `+through_wall_radar`); CP37 mig-0030 further extended it 16 → 17 (`+network_surveillance`) — dual-table CHECK literal sweeps continuing the CP32 §1 precedent; 0 row promotions land through v1.6.0 (schema slots open for future evidence-arrival) |
