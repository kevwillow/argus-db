...
|---|---:|---|
| `identifiers` | **41,716 active** + 342 chained-superseded + 40 self-loop withdrawn-no-successor (4 pre-MAC-217 PII demotes + 36 MAC-291 §11 #1 strip demotes) | active = `superseded_by IS NULL` = **41,716**; total = **42,098**; demoted = 382 (= 346 pre-MAC-291 baseline + 36 MAC-291 strip; unchanged at v1.6.6 — the +208 are all active INSERTs). CP32 §9 tri-state semantic preserved (`NULL` = active / `<other_id>` = superseded by successor / `<self_id>` = withdrawn-no-successor PII-demote or §11 #1 strip-demote). Cumulative active deltas: +1 in v1.1.0 (Johnson Matthey PLC); +16 in v1.2.0 (MAC-178 P3); +12,239 in v1.4.0 (Wave I/I.5/I.6/I.7 hostname corpus); +172 in v1.4.1 (Stage 1 SAR-15.5 PASS); +848 in v1.5.0 (Stage 1 Step 5 promotions via [MAC-232](/MAC/issues/MAC-232) lexicon-expansion wave); +146 in v1.5.2 (CCTV cohort); +1,843 in v1.5.3 (Phase H, [MAC-250](/MAC/issues/MAC-250)); +3,627 in v1.6.0 (Wave K + Wave L combined ship, [MAC-275](/MAC/issues/MAC-275)); **+116 in v1.6.2 ([MAC-288](/MAC/issues/MAC-288) MAC-279 Phase-6 substantive promotion: range id 42593–42708 contiguous)**; **−36 in v1.6.2 ([MAC-291](/MAC/issues/MAC-291) §11 #1 strip demotion of Wave G/H v1 CCTV placeholder rows; 18 OUI + 18 NDPP)** — net v1.6.2 = +80; **+208 in v1.6.6 ([MAC-321](/MAC/issues/MAC-321) WS-1 registry promotion: range id 42709–42916 contiguous)** — net v1.6.6 = +208 |
...
The canonical Argus identifier table. Every row represents one identifier-to-attribution binding. Row count verified live 2026-06-06 (v1.6.6 ship, post-MAC-321 WS-1 registry promotion): **41,716 active** (`superseded_by IS NULL`); 342 chained-superseded (`superseded_by` points at a successor row) + 40 self-loop withdrawn-no-successor (`superseded_by = id`; 4 pre-MAC-217 PII-demotes + 36 MAC-291 §11 #1 strip-demotes) preserved per the CP32 §9 tri-state semantic. Total = 42,098; demoted total = 382 (unchanged at v1.6.6 — the +208 are all active INSERTs). v1.6.6 net delta: +208 active ([MAC-321](/MAC/issues/MAC-321) WS-1 registry promotion; range id 42709–42916 contiguous; oui +133, mac_range +19, fcc_grantee_code +44, vendor_controlled_hostname +5, firmware_sha256_hash +3, product_family_codename +2, firmware_branded_string +2). Prior v1.6.2 net delta was +80 active (= +116 from MAC-279 Phase-6 substantive promotion (MAC-288; range id 42593–42708 contiguous) − 36 from MAC-291 §11 #1 strip; 18 OUI + 18 NDPP placeholder demotes).
